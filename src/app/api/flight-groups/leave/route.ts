...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Attempting to leave flight group:', {
      userId: user.id,
      groupId: group_id,
    ...
    if (deleteError) {
      ...
      return NextResponse.json(
        { error: 'Failed to leave flight group: ' + deleteError.message },
        { status: 500 }
      )
    }
    ...
    if (verifyError) {
      ...
      return NextResponse.json(
        { error: 'Failed to leave flight group - deletion unsuccessful' },
        { status: 500 }
      )
    } else {
    ...
    return NextResponse.json({
      success: true,
      message: 'Successfully left the flight group'
    })

  } catch (error) {
    console.error('Leave flight group error:', error)
    return NextResponse.json(
      { error: 'Failed to leave flight group' },
      { status: 500 }
    )
  }
}